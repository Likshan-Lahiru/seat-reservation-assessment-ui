...
export function HomePage() {
  ...
  return <div className="min-h-screen bg-luxury-black pb-20">
      {/* Hero Section */}
      <section className="relative h-screen w-full overflow-hidden">
        <div className="absolute inset-0">
          <img src={FEATURED_MOVIE.image} alt={FEATURED_MOVIE.title} className="w-full h-full object-cover" />
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="flex items-end justify-between mb-12">
          ...
          <div className="hidden md:flex gap-4">
            {['All', 'Action', 'Drama', 'Sci-Fi'].map((filter, i) => <button key={filter} className={`text-sm uppercase tracking-wider transition-colors ${i === 0 ? 'text-luxury-gold font-bold' : 'text-gray-500 hover:text-white'}`}>
                {filter}
              </button>)}
          </div>
        </div>
        ...
        {!loading && !error && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {movies.map((movie, index) => <MovieCard key={movie.id} id={movie.id} title={movie.title} image={movie.image_url} genre="Drama" rating={8.5} duration="2h 30m" showtimes={movie.shows.length} index={index} />)}
          </div>}
      </section>
