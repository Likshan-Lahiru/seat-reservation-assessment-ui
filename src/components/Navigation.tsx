...
export function Navigation() {
    ...
    const hideLogo =
        location.pathname.startsWith('/seat') ||
    ...
    return (
        <header
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
                isScrolled
                    ? 'bg-luxury-black/90 backdrop-blur-md border-b border-luxury-gold/20 py-4'
                    : 'bg-transparent py-6'
            }`}
        >
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                {/* Logo (hidden on seat pages) */}
                {!hideLogo ? (
              ...
              BOOKMY<span className="text-luxury-gold">TICKET</span>
                    ...
                    // keeps spacing so layout doesn't jump
